...
- fork 时不复制任何可写内存，父子进程的虚拟地址都映射到**同一份物理内存**，共享着用。
- 只要双方都只是读，就一直共享，零复制开销。
- 只有当某一方要写某个页时，才真正为它复制出一份私有副本，让它在副本上写。
于是：如果 fork 后立刻 execve（根本没怎么写旧内存），几乎不发生任何复制
...
- 策略（Policy）: 做什么决定/选择哪一个
- 机制（Mechanism）: 如何实现该决定
![1782394119941](image/final_review/1782394119941.png)
...
#### 批处理任务的调度（没人一直盯着等结果，需要的是吞吐量和周转时间）
...
Fast path
- Per CPU从当前slab(current)中取出一个适合大小的块（快速，没有和其他CPU的竞争）
low path
- 需要从一个全局的partial里去找一个作为当前的slab(current)，不巧的话（paritial里也没空闲内存），甚至需要从buddy system里重新分配连续空间，再次分割为可用的slab
...
![1782398335367](image/final_review/1782398335367.png)
![1782398190779](image/final_review/1782398190779.png)
处理缺页异常的三个主要活动：
- 服务中断：一般只需要几百条指令
- 读取页面：需要大量时间
- 恢复进程：需要少量时间
...
### inode多级索引
![1782400237736](image/final_review/1782400237736.png)
![1782400243687](image/final_review/1782400243687.png)
![1782400249517](image/final_review/1782400249517.png)
![1782400260846](image/final_review/1782400260846.png)
这里因为数据块是4KB，一个表项是4B，所以一个表可以存储1024个数据块的地址
![1782400420456](image/final_review/1782400420456.png)
